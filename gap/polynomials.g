...
# Input: 	collector coll
# Output:	none, but polynomials f_rs are added to the collector coll 
# Note: the code is almost identical to the code of DTP_DTpols_rs, only that 
# the existing collector is manipulated. 
InstallGlobalFunction( DTP_AddDTPolsToCollector,
function(coll)
	local isConfl, n, s, all_pols, orders, gen, DTObj;
	...
	Objectify(DTObjType, coll);
	# TODO: Problem!?! IsDTObj(coll) = false after calling this function
	...
	coll![PC_DTPPolynomials] := all_pols; 
	coll![PC_DTPOrders] := [1 .. n] * infinity; # for computing the generator 
	# orders we also need to provide "orders", since we use the same 
	# functions for multiplication. Hence, first assume them to be infinite. 
	# Then during multiplication we do not reduce any results. 
	
	coll![PC_DTPConfluent] := isConfl; 
	
	if isConfl then 
		DTP_OrdersGenerators(coll); 
		for s in [1 .. n] do 
			DTP_ReducePolynomialsModOrder(coll![PC_DTPPolynomials][s], coll![PC_DTPOrders]); 
		od; 
	else 
		# We will use the same function for multiplication (DTP_Multiply_s) 
		# as in the case, when the generator orders are provided. The 
